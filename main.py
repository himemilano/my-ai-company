...
# 💻 アプリ開発部のAIスペシャリスト（完全プロフェッショナル仕様）
dev_pm = Agent(
...
# 🚀 タスクの定義（プロジェクトの配置パスをAIへ厳密に伝達）
task1 = Task(
    description=f"""
【最重要：開発対象アプリ】: {proj_config['project_title']}
【ソースコードおよび関連ファイル配置エリア】: {PROJECT_ROOT} 内のすべて
【専用ナレッジベース】:\n{shared_knowledge_base}

現在の開発ステート（{state['current_phase']}）および上記フォルダ内の既存コードを確認してください。
本日開発・修正すべき『具体的なFlutterの1画面、または1つのロジック』を厳選して決定し、エンジニアに {PROJECT_ROOT} 内のどのファイルをどう修正すべきか指示を出してください。
""",
...
task2 = Task(
    description=f"PMの指示、および先述の仕様書・専門ナレッジの内容を基に、アプリに必要なDartコード等を正確に実装・修正してください。ファイルの書き出しや修正は必ず {PROJECT_ROOT} 内の適切な構造（lib/等）に対して行うこと。",
    expected_output="実装または修正されたDart/JSONコード一式（Markdownのコードブロック形式）",
    agent=flutter_engineer
)

task3 = Task(
    description="エンジニアが実装したコードをレビューし、仕様書や専門知識のロジック通りに作られているかチェックしてください。最後に、次回の出勤メンバーへの『明日の引き継ぎ事項』をまとめてください。",
...
try:
    ...
    sys.exit(1) # Actionsの無限ループを即座に遮断
