...
# 📖 ローカルMD ＋ 指定URLのWebスクレイピングからハイブリッドにナレッジをロード
def load_project_knowledge_base():
    combined_context = ""
    loaded_count = 0
    
    # 1. ローカルのMarkdownファイルをスキャン
    md_files = glob.glob(os.path.join(KNOWLEDGE_DIR, "*.md"))
    print(f"📂 ターゲット [ {proj_config['project_title']} ] のローカルナレッジをスキャン中...")
    ...
    for file_path in md_files:
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                content = f.read()
                combined_context += f"\n\n=========================================\n"
                combined_context += f"📄 参照ローカルファイル: {os.path.basename(file_path)}\n"
                combined_context += f"=========================================\n"
    ...
    # 2. 指定された外部URLから自律スクレイピング（対策B）
    urls = proj_config.get("knowledge_urls", [])
    if urls:
        print(f"🌐 設定された外部Webサイト（計 {len(urls)} 件）から過去問・知識データベースをダウンロード中...")
        for url in urls:
            try:
                # 403ブロックを防ぐために一般的なブラウザのUser-Agentを偽装設定
                ...
                web_text = extractor.get_text()
                
                combined_context += f"\n\n=========================================\n"
                combined_context += f"🌐 参照外部Webデータ: {url}\n"
                combined_context += f"=========================================\n"
                combined_context += web_text
                print(f"✨ 外部URLの解析に成功しました: {url[:60]}...")
                loaded_count += 1
            except Exception as e:
                print(f"⚠️ URL {url} からの知識取得に失敗（スキップします）: {e}")
            
    if not combined_context:
        return "プロジェクト固有のナレッジ（.md または URL）が検知されませんでした。"
        
    print(f"🎯 計 {loaded_count} 個のハイブリッド・ナレッジベースを正常にロードしました。")
...
dev_pm = Agent(
    ...
    goal=f"支給された仕様書・Webから取得した専門過去問ナレッジと既存コードを紐解き、{proj_config['project_title']} をストア申請に向けて着実に完成させる",
...
【ハイブリッド・ナレッジベース（外部Webの地質調査過去問解説を含む）】:\n{shared_knowledge_base}
...
生成する「QA合格承認ログ」の承認日や、「明日への引き継ぎメモ」の日付には、過去の古いいかなるテンプレートの日付（2023年など）も決して使わず、必ず本日の日付【 {current_date} 】を正確に記載してください。
""",
